...
### (선택) 개인용 모드: 내 모든 파일을 링크로 바로 열기

기본 범위(`drive.file`)는 Picker 로 **선택한 파일에만** 접근합니다. 그래서 로그인해도
비공개 파일을 링크 붙여넣기로 열 수는 없습니다(선택한 적 없는 파일이므로).

혼자 쓰는 뷰어라면 `config.js` 에서 `READONLY_SCOPE: true` 로 바꾸세요.
로그인 범위가 `drive.readonly` 로 확장되어, **내 드라이브의 아무 파일이나
공유 설정 없이 링크 붙여넣기로 바로** 열 수 있습니다(Picker 도 그대로 동작).

- `drive.readonly` 는 제한(restricted) 범위입니다. 앱을 일반 공개하려면 구글 심사가
  필요하지만, OAuth 동의 화면이 **테스트** 상태이고 본인이 **테스트 사용자**로 등록되어
  있으면 심사 없이 사용할 수 있습니다.
- 로그인 시 "Google 에서 확인하지 않은 앱" 경고가 나오면 **고급 → (앱 이름)(안전하지 않음)으로 이동** 을 누르면 됩니다. 본인이 만든 앱이므로 안전합니다.
